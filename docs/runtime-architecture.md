...
2. **Vorwärtsreferenzen:** `ReplAnalyzer` kapselt vorerst den vorhandenen
   ...
   Deklarationen noch anhand von Fehlermeldungen. Eine vorwärts referenzierte
   Klasse wird direkt vor die Top-Level-Deklaration gesetzt, deren Analyse sie
   brauchte (Fehlerposition bzw. Oberklassenbezug), damit ihre eigenen
   Abhängigkeiten weiterhin davor stehen. Jeder Versuch kostet einen weiteren
   vollständigen Analyse-Durchlauf. Das ist verbleibende technische
