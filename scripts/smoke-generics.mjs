...
// Runtime type/resolver caches are keyed by class identity and only used for
// concrete type arguments. Alternating element types, nullability, loops and a
// type parameter that shadows a class name must still resolve independently.
const cacheSession = api.bluekCreateKotliteSession();
load(cacheSession, `
    open class Animal
    class Cat : Animal()
    class Item(val id: Int)
    fun <T> wrap(value: T): List<T> = listOf(value)
    fun <Item> shadow(value: Item): List<Item> = listOf(value)
    fun <T> firstOf(items: List<T>): T { for (item in items) return item; throw Exception("empty") }
`, 'runtime type caches');
if (evaluate(cacheSession, `
    val ints = wrap(1)
    val texts = wrap("x")
    val cats = wrap(Cat())
    val nullable: List<Int?> = listOf(null, 2)
    val plain: List<Int> = listOf(3)
    var sum = 0
    for (value in listOf(1, 2)) sum += value
    var joined = ""
    for (value in listOf("a", "b")) joined += value
    var catCount = 0
    for (animal in listOf<Animal>(Cat(), Animal())) if (animal is Cat) catCount += 1
    val items: List<Item> = listOf(Item(4))
    ints[0] + 1 == 2 && texts[0].length == 1 && cats[0] is Cat && firstOf(texts) == "x" &&
        nullable[0] == null && nullable[1] == 2 && plain[0] == 3 && sum == 3 && joined == "ab" &&
        catCount == 1 && shadow(7)[0] + 1 == 8 && items[0].id == 4 && firstOf(items).id == 4
`, 'runtime type caches').display !== 'true') {
  throw new Error('Cached runtime types mixed up concrete arguments, nullability or a shadowing type parameter.');
}
