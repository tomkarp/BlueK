...
try {
  ...
  const files = [
    ...
    { id: 'base', fileName: 'Base.kt', kind: 'class', revision: 1, source: 'open class Base(var baseValue: String, var baseCount: Int = 0)' },
  ...
  const hiddenApi = compiled.body.classes.find(value => value.name === 'HiddenApi');
  assert.deepEqual(hiddenApi.properties.map(value => value.name), []);
  assert.equal(hiddenApi.methods.some(value => value.name === 'implementationCall'), false);
  ...
  const childMethods = compiled.body.classes.find(value => value.name === 'Child').methods;
  assert.ok(childMethods.some(value => value.name === 'getBaseCount' && value.inheritedFrom === 'Base' && value.autoGenerated));
  assert.ok(childMethods.some(value => value.name === 'setBaseCount' && value.inheritedFrom === 'Base' && value.autoGenerated));
  const actorMethods = compiled.body.classes.find(value => value.name === 'Actor').methods;
  assert.ok(actorMethods.some(value => value.name === 'isClicked' && value.autoGenerated && value.returnType.displayName === 'Boolean'));
  assert.equal(actorMethods.some(value => value.name === 'getIsClicked'), false);
