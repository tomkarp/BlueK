...
- Rechtsklick auf eine `«functions»`-Karte zeigt Top-Level-Funktionen; ihre Kotlin-Argumente werden genauso compilergeprüft eingegeben. Ein parameterloses `main()` oder ein übliches `main(args: Array<String>)` kann dort oder über den direkten `Run main`-Befehl wie in BlueJ ausgeführt werden.
