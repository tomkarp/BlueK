...
const bindingValueWithBridges = (descriptor) => { if (descriptor?.bridge) return (...args) => api[descriptor.bridge](...args.map(arg => arg && arg.__bluekObjectId ? objects.get(arg.__bluekObjectId) : arg)); if (descriptor?.bridges) return (...args) => { const bridge = descriptor.bridges.find(candidate => candidate.arity === args.length) || descriptor.bridges[0]; if (!bridge || typeof api[bridge.name] !== 'function') throw new Error('Generated Kotlin bridge is missing.'); return api[bridge.name](...args.map(arg => arg && arg.__bluekObjectId ? objects.get(arg.__bluekObjectId) : arg)); }; return bindingValue(descriptor); };
