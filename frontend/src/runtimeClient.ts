...
const objectPropertyNames = new Map();
const objectPropertyDescriptors = new Map();
let outputBuffer = '';
...
const bindingTargets = new WeakMap();
const valueOf = (value, className = '') => {
  ...
  const binding = typeof value === 'object' && value !== null ? bindingTargets.get(value) : undefined; const storedValue = binding?.target || value; const objectId = crypto.randomUUID(); objects.set(objectId, storedValue); if (binding?.propertyNames?.length) objectPropertyNames.set(objectId, binding.propertyNames); if (binding?.properties?.length) objectPropertyDescriptors.set(objectId, binding.properties); const display = className || storedValue.constructor?.name || 'Object'; displayValues.set(objectId, display); return { kind: 'object', display, objectId };
};
...
self.onmessage = async ({ data }) => {
  ...
  try {
    ...
    if (data.op === 'run') { const run = runtimeApi?.bluekRun || runtimeApi?.start || api.bluekRun || resolve('bluekRun') || resolve('start'); if (typeof run !== 'function') throw new Error('Browser BluePlay runtime is missing its run bridge.'); run(); clearTimeout(simulationTimer); clearInterval(renderTimer); simulationAccumulator = 0; lastSimulationTime = performance.now(); const tick = runtimeApi?.bluekStep || resolve('bluekStep'); const isRunning = runtimeApi?.bluekIsRunning || resolve('bluekIsRunning'); const stopTimers = () => { clearTimeout(simulationTimer); clearInterval(renderTimer); simulationTimer = null; renderTimer = null; simulationAccumulator = 0; lastSimulationTime = 0; }; const simulate = () => { const now = performance.now(); const elapsed = Math.min(100, Math.max(0, now - lastSimulationTime)); lastSimulationTime = now; simulationAccumulator += elapsed; const speed = Number(runtimeApi?.getSpeed?.() || resolve('getSpeed')?.() || 50); const stepMilliseconds = Math.max(8, 1000 / (5 + speed * 1.1)); while (simulationAccumulator >= stepMilliseconds) { if (typeof tick === 'function') tick(); simulationAccumulator -= stepMilliseconds; if (typeof isRunning === 'function' && !isRunning()) { stopTimers(); stage(); return; } } simulationTimer = setTimeout(simulate, 16); }; simulationTimer = setTimeout(simulate, 0); renderTimer = setInterval(stage, 16); stage(); self.postMessage({ kind: 'value', value: { kind: 'unit', display: 'Unit' } }); return; }
    ...
    if (data.op === 'pause') { const pause = runtimeApi?.bluekPause || resolve('bluekPause'); if (typeof pause === 'function') pause(); clearTimeout(simulationTimer); clearInterval(renderTimer); simulationTimer = null; renderTimer = null; simulationAccumulator = 0; lastSimulationTime = 0; stage(); finish(); return; }
    ...
    if (data.op === 'get') { const object = objects.get(data.objectId); if (!object) throw new Error('Object handle is no longer available.'); const classKey = String(data.className || '').replace(/[^A-Za-z0-9_]/g, '_'); const stem = String(data.property).charAt(0).toUpperCase() + String(data.property).slice(1); const getter = api['bluekInvoke_' + classKey + '_get' + stem + '_noargs']; if (typeof getter !== 'function') throw new Error('The Kotlin property ' + data.property + ' is not declared.'); const value = getter(object); const output = takeOutput(); self.postMessage({ kind: 'value', value: valueOf(value), output }); return; }
    if (data.op === 'set') { const object = objects.get(data.objectId); if (!object) throw new Error('Object handle is no longer available.'); const classKey = String(data.className || '').replace(/[^A-Za-z0-9_]/g, '_'); const stem = String(data.property).charAt(0).toUpperCase() + String(data.property).slice(1); const setterKey = Object.keys(api).find((key) => key.startsWith('bluekInvoke_' + classKey + '_set' + stem + '_')); const setter = setterKey ? api[setterKey] : null; if (typeof setter !== 'function') throw new Error('The Kotlin property ' + data.property + ' is not declared or is read-only.'); const value = resolveArguments([data.value])[0]; setter(object, value); stage(); finish(); return; }
    if (data.op === 'inspect') { const object = objects.get(data.objectId); if (!object) throw new Error('Object handle is no longer available.'); const entries = Object.entries(object).filter(([key]) => !key.startsWith('$')); const namesFn = data.className ? (api['bluekInspectNames_' + data.className] || resolve('bluekInspectNames_' + data.className)) : null; let names = Array.isArray(data.propertyNames) && data.propertyNames.length ? data.propertyNames : (objectPropertyNames.get(data.objectId) || []); if (!names.length && typeof namesFn === 'function') { try { names = JSON.parse(namesFn()); } catch { names = []; } } const fieldNames = names.length ? names : entries.map(([key]) => key); const descriptors = new Map((objectPropertyDescriptors.get(data.objectId) || []).map((property) => [property.name, property])); const classKey = String(data.className || '').replace(/[^A-Za-z0-9_]/g, '_'); const getterValue = (name) => { const descriptor = descriptors.get(name); const getter = descriptor?.getter ? api[descriptor.getter] : api['bluekInvoke_' + classKey + '_get' + name.charAt(0).toUpperCase() + name.slice(1) + '_noargs']; if (typeof getter !== 'function') return undefined; try { return getter(object); } catch { return undefined; } }; const fields = fieldNames.map(name => { const value = getterValue(name); const entry = entries.find(([key]) => key === name); return { name, value: valueOf(value === undefined ? entry?.[1] : value).display }; }); self.postMessage({ kind: 'value', value: { kind: 'object', objectId: data.objectId, fields } }); return; }
    if (data.op === 'remove') { objects.delete(data.objectId); objectPropertyNames.delete(data.objectId); objectPropertyDescriptors.delete(data.objectId); displayValues.delete(data.objectId); stage(); self.postMessage({ kind: 'value', value: { kind: 'unit', display: 'Unit' } }); return; }
...
export class HybridRuntimeClient implements RuntimeClient {
  ...
  private readonly pendingLocalRejectors = new Set<(error: Error) => void>();
  ...
  private startBrowserWorker(): void {
    ...
    this.cancelPendingLocal('Browser Kotlin/JS worker replaced.');
  ...
  private cancelPendingLocal(message: string): void {
    const error = new Error(message);
    for (const reject of [...this.pendingLocalRejectors]) reject(error);
    this.pendingLocalRejectors.clear();
  }
  private async local(request: BrowserAction): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pendingLocalRejectors.add(rejectPending);
  ...
  async execute(request: Action): Promise<Value & { output?: string; stage?: unknown; name?: string }> {
    ...
    if (this.worker && this.ready && request.op === 'create') {
      ...
      if (klass && constructor) {
        const args = JSON.parse(String(request.args || '[]')).map((value: string) => parseKotlinArgument(value, this.bindings, this.localValues)); if (!simpleCallableMatches(args, constructor.parameters)) throw new Error('The Kotlin constructor was called with the wrong number or type of arguments.'); const functionName = `bluekCreate_${klass.name.replace(/[^A-Za-z0-9_]/g, '_')}${klass.constructors.indexOf(constructor) === 0 ? '' : `_${klass.constructors.indexOf(constructor)}`}`;
    ...
    if (this.worker && this.ready && request.op === 'inspect') {
      const objectId = String(request.objectId); const className = String(this.localObjects.get(objectId) || ''); const klass = this.classes.find(value => value.name === className); const propertyNames = (klass?.properties || []).map(property => property.name); return (await this.local({ op: 'inspect', objectId, className, propertyNames })).value;
    }
