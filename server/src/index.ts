...
function enrichProperties(classes:ClassMeta[],files:ProjectFile[]):ClassMeta[]{return classes.map(value=>{if(value.kind!=='class')return value;const file=files.find(item=>item.fileName.replace(/\.kt$/,'')===value.name);if(!file)return value;const source=maskKotlinNonCode(file.source);const declaration=source.search(new RegExp(`\\b(class|abstract\\s+class)\\s+${value.name}\\b`));const bodyStart=declaration<0?-1:source.indexOf('{',declaration);const headerEnd=declaration<0?-1:source.indexOf(')',declaration);const depthAt=(index:number)=>{let depth=0;for(let position=0;position<index;position++){if(source[position]==='{')depth++;else if(source[position]==='}')depth=Math.max(0,depth-1)}return depth};const properties:any[]=[];const seen=new Set<string>();for(const match of source.matchAll(/\b(public|private|protected|internal)?\s*(val|var)\s+(\w+)(?:\s*:\s*([^=\n;,\)]+))?(?:\s*=\s*([^\n;]+))?/g)){const index=match.index||0;const inPrimaryHeader=index>declaration&&((bodyStart>=0&&index<bodyStart)||(bodyStart<0&&index<headerEnd));const isMember=bodyStart>=0&&index>bodyStart&&depthAt(index)===1;if(!inPrimaryHeader&&!isMember)continue;const visibility=(match[1]||'public').trim();if(hiddenKotlinMember(source,index)||visibility!=='public'||seen.has(match[3]))continue;seen.add(match[3]);const propertyType=(match[4]||inferredPropertyType(match[5])).trim().replace(/\s+(?:get|set)\s*\(.*$/,'').trim();properties.push({id:`${value.name}::property::${match[3]}`,name:match[3],type:tr(propertyType),mutable:match[2]==='var',visibility})}return {...value,properties}})}
