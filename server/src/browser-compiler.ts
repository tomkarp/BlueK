...
function bridgeSource(files: ProjectFile[], classes: ClassMeta[]): { source: string; packageName: string } {
    ...
    for (const klass of classes.filter(value => value.kind === 'class' && value.constructors.length)) {
        ...
        for (const method of klass.methods.filter(value => value.visibility === 'public' && !('autoGenerated' in value) && !value.typeParameters?.length && !value.returnType.displayName.startsWith('List') && !value.parameters.some(parameter => parameter.type.displayName.startsWith('List')))) {
            ...
            lines.push('', '@OptIn(ExperimentalJsExport::class)', '@JsExport', `fun bluekInvoke_${className}_${requiredKey}(receiver: ${klass.name}${args.length ? `, ${args.join(', ')}` : ''}): ${returnType} = receiver.${method.name}(${args.map((_, index) => `arg${index}`).join(', ')})`);
        }
    }
    for (const owner of classes.filter(value => value.kind === 'functions')) {
        for (const method of owner.methods.filter(value => value.visibility === 'public' && !value.typeParameters?.length && !value.returnType.displayName.startsWith('List') && !value.parameters.some(parameter => parameter.type.displayName.startsWith('List')))) {
            const args = requiredParameters(method.parameters || []).map((parameter, index) => `arg${index}: ${kotlinType(parameter.type.displayName)}`);
            const returnType = kotlinType(method.returnType.displayName);
            const requiredKey = bridgeMethodKey({ ...method, parameters: requiredParameters(method.parameters || []) });
            lines.push('', '@OptIn(ExperimentalJsExport::class)', '@JsExport', `fun bluekCall_${kotlinIdentifier(owner.name)}_${requiredKey}(${args.join(', ')}): ${returnType} = ${method.name}(${args.map((_, index) => `arg${index}`).join(', ')})`);
        }
    }
