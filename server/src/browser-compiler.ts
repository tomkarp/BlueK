...
function bridgeSource(files: ProjectFile[], classes: ClassMeta[]): { source: string; packageName: string } {
    ...
    for (const klass of classes.filter(value => value.kind === 'class' && value.constructors.length)) {
        ...
        const fieldJson = `[${fieldNames.map(name => `"${name}"`).join(',')}]`;
    ...
    for (const owner of classes.filter(value => value.kind === 'functions')) {
        for (const method of owner.methods.filter(value => value.visibility === 'public' && !value.typeParameters?.length && !value.parameters.some(parameter => parameter.hasDefault) && !value.returnType.displayName.startsWith('List') && !value.parameters.some(parameter => parameter.type.displayName.startsWith('List')))) {
            const args = (method.parameters || []).map((parameter, index) => `arg${index}: ${kotlinType(parameter.type.displayName)}`);
            const returnType = kotlinType(method.returnType.displayName);
            lines.push('', '@OptIn(ExperimentalJsExport::class)', '@JsExport', `fun bluekCall_${kotlinIdentifier(owner.name)}_${bridgeMethodKey(method)}(${args.join(', ')}): ${returnType} = ${method.name}(${args.map((_, index) => `arg${index}`).join(', ')})`);
        }
    }
