...
import type { ClassMeta, ProjectFile, SymbolManifest, TypeRef } from '../../runtime-contract/src/index.js';
...
const unsupportedBridgeType = (displayName: string) => /\bLong\b/.test(displayName) || /\([^)]*\)\s*->/.test(displayName);
...
function bridgeSource(files: ProjectFile[], classes: ClassMeta[]): { source: string; packageName: string } {
    ...
    for (const klass of classes.filter(value => value.kind === 'class' && value.constructors.length && !frameworkNames.has(value.name))) {
        ...
        for (const method of klass.methods.filter(value => value.visibility === 'public' && !value.typeParameters?.length && !(value.inheritedFrom && frameworkNames.has(value.declaringType)) && !unsupportedBridgeType(value.returnType.displayName) && !value.parameters.some(parameter => parameter.type.displayName.startsWith('List') || unsupportedBridgeType(parameter.type.displayName)))) {
    ...
    for (const owner of classes.filter(value => value.kind === 'functions')) {
        for (const method of owner.methods.filter(value => value.declaringType === owner.name && value.visibility === 'public' && !value.typeParameters?.length && !unsupportedBridgeType(value.returnType.displayName) && !value.parameters.some(parameter => parameter.type.displayName.startsWith('List') || unsupportedBridgeType(parameter.type.displayName)))) {
...
const simpleManifestType = (displayName: string): string => {
    ...
    if (genericStart >= 0 && base.endsWith('>')) {
        const name = base.slice(0, genericStart).split('.').at(-1) || 'Any';
        return name + '<' + base.slice(genericStart + 1, -1).split(',').map(simpleManifestType).join(', ') + '>' + nullable;
    }
    return (base.split('.').at(-1) || 'Any') + nullable;
};

const manifestTypeDisplay = (value: unknown): string => {
    if (typeof value === 'string') return value;
    if (value && typeof value === 'object' && 'displayName' in value && typeof value.displayName === 'string') return value.displayName;
    return 'Any';
};

const manifestTypeRef = (value: unknown): TypeRef => {
    if (value && typeof value === 'object') {
        const record = value as { classifier?: unknown; arguments?: unknown; nullable?: unknown; displayName?: unknown; projection?: unknown };
        const classifier = simpleManifestType(typeof record.classifier === 'string' ? record.classifier : 'Any');
        const argumentsList = Array.isArray(record.arguments) ? record.arguments.map(manifestTypeRef) : [];
        const displayName = simpleManifestType(typeof record.displayName === 'string' ? record.displayName : classifier);
        const projection = record.projection === 'in' || record.projection === 'out' || record.projection === 'star' ? record.projection : undefined;
        return { classifier, arguments: argumentsList, nullable: record.nullable === true, displayName, ...(projection ? { projection } : {}) };
    }
    const displayName = manifestTypeDisplay(value);
    return { classifier: simpleManifestType(displayName), arguments: [], nullable: displayName.trim().endsWith('?'), displayName: simpleManifestType(displayName) };
};
...
export const classesFromManifest = (manifest: SymbolManifest): ClassMeta[] => manifest.classes.map(declaration => ({
    ...
    constructors: declaration.constructors.map((constructor, constructorIndex) => ({
        ...
        parameters: constructor.parameters.map((parameter, index) => ({
            name: parameter.name || 'arg' + index,
            type: manifestTypeRef(parameter.type),
            hasDefault: parameter.hasDefault === true,
        })),
    })),
    methods: declaration.methods.filter(method => !method.generated).map((method, methodIndex) => ({
        id: declaration.name + '::method::' + methodIndex,
        name: method.name,
        declaringType: method.declaringType.split('.').at(-1) || declaration.name,
        parameters: method.parameters.map((parameter, index) => ({
            name: parameter.name || 'arg' + index,
            type: manifestTypeRef(parameter.type),
            hasDefault: parameter.hasDefault === true,
        })),
        returnType: manifestTypeRef(method.returnType),
        visibility: 'public',
        inheritedFrom: method.declaringType.split('.').at(-1) !== declaration.name ? method.declaringType.split('.').at(-1) : undefined,
        typeParameters: method.typeParameters,
        autoGenerated: method.generated,
    } as ClassMeta['methods'][number] & { autoGenerated?: boolean })).concat(declaration.properties.flatMap(property => {
        const existing = new Set(declaration.methods.filter(method => !method.generated).map(method => method.name));
        const type = manifestTypeRef(property.type);
        const booleanIsProperty = type.classifier === 'Boolean' && property.name.startsWith('is');
        const stem = property.name.charAt(0).toUpperCase() + property.name.slice(1);
        const getterName = booleanIsProperty ? property.name : `get${stem}`;
        const accessors: any[] = [];
        if (property.getter !== false && !existing.has(getterName)) accessors.push({
            id: `${declaration.name}::property-getter::${property.name}`,
            name: getterName,
            declaringType: declaration.name,
            parameters: [],
            returnType: type,
            visibility: 'public',
            autoGenerated: true,
        ...
        if (property.mutable && property.setter !== false) {
            const setterName = `set${booleanIsProperty ? property.name.slice(2) : stem}`;
            if (!existing.has(setterName)) accessors.push({
                id: `${declaration.name}::property-setter::${property.name}`,
                name: setterName,
                declaringType: declaration.name,
                parameters: [{ name: 'value', type, hasDefault: false }],
                returnType: { classifier: 'Unit', arguments: [], nullable: false, displayName: 'Unit' },
                visibility: 'public',
                autoGenerated: true,
                propertyName: property.name,
            });
    ...
    properties: declaration.properties.map(property => ({
        id: declaration.name + '::property::' + property.name,
        name: property.name,
        type: manifestTypeRef(property.type),
        mutable: property.mutable,
        visibility: 'public',
    })),
    supertypes: declaration.supertypes.map(manifestTypeRef),
...
export async function compileBrowserProject(root: string, sessionDir: string, files: ProjectFile[]): Promise<{ ok: boolean; diagnostics: string; directory: string; manifest?: SymbolManifest }> {
   ...
   try {
        await fs.copyFile(path.join(buildDir, 'manifest.json'), path.join(dist, 'bluek-symbol-manifest.json'));
    } catch {
