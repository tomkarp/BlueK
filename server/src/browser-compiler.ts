...
export const classesFromManifest = (manifest: SymbolManifest): ClassMeta[] => manifest.classes.map(declaration => ({
    ...
    constructors: declaration.constructors.map((constructor, constructorIndex) => ({
        ...
        parameters: constructor.parameters.map((parameter, index) => ({
            ...
            type: {
                classifier: simpleManifestType(manifestTypeDisplay(parameter.type)),
                arguments: [],
                nullable: manifestTypeDisplay(parameter.type).trim().endsWith('?'),
                displayName: simpleManifestType(manifestTypeDisplay(parameter.type)),
            },
    ...
    methods: declaration.methods.filter(method => !method.generated).map((method, methodIndex) => ({
        ...
        returnType: {
            classifier: simpleManifestType(manifestTypeDisplay(method.returnType)),
            arguments: [],
            nullable: manifestTypeDisplay(method.returnType).trim().endsWith('?'),
            displayName: simpleManifestType(manifestTypeDisplay(method.returnType)),
        },
        visibility: 'public',
        ...
        const type = {
            classifier: simpleManifestType(manifestTypeDisplay(property.type)),
            arguments: [],
            nullable: manifestTypeDisplay(property.type).trim().endsWith('?'),
            displayName: simpleManifestType(manifestTypeDisplay(property.type)),
        };
    ...
    supertypes: declaration.supertypes.map(type => ({
        classifier: simpleManifestType(manifestTypeDisplay(type)),
        arguments: [],
        nullable: manifestTypeDisplay(type).trim().endsWith('?'),
        displayName: simpleManifestType(manifestTypeDisplay(type)),
    })),
