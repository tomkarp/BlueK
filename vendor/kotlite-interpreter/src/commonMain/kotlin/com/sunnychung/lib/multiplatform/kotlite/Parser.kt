...
import com.sunnychung.lib.multiplatform.kotlite.model.ImportDirective
...
//        repeatedNL() // this would cause the NL before the next statement not recognized
        ...
        // `private` only belongs to this property when an accessor follows; otherwise it is the
        // modifier of the next class member and must not be consumed here.
        val setterIsPrivate = nextToken.type == TokenType.Identifier && nextToken.value == "private" && run {
            val originalTokenIndex = tokenIndex
            repeatedNL()
            eat(TokenType.Identifier, "private")
            repeatedNL()
            val afterPrivate = currentToken
            val isAccessor = afterPrivate.type == TokenType.Identifier && afterPrivate.value in setOf("get", "set")
            resetTokenToIndex(originalTokenIndex)
            isAccessor
        }
        if (setterIsPrivate) {
...
//        } while (curr != null)
    ...
    private fun importDirective(): ImportDirective {
        val position = eat(TokenType.Identifier, "import").position
        val path = StringBuilder(eat(TokenType.Identifier).value as String)
        while (isCurrentToken(TokenType.Operator, ".")) {
            eat(TokenType.Operator, ".")
            path.append('.')
            path.append(if (isCurrentToken(TokenType.Operator, "*")) eat(TokenType.Operator, "*").value as String else eat(TokenType.Identifier).value as String)
        }
        val alias = if (isCurrentToken(TokenType.Identifier, "as")) {
            eat(TokenType.Identifier, "as")
            eat(TokenType.Identifier).value as String
        } else null
        return ImportDirective(position, path.toString(), alias)
    }
