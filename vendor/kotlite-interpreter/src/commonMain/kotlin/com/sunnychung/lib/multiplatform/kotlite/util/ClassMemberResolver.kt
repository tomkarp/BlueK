...
//            classTreeIndex = index,
    ...
    // The resolver is immutable after construction, so resolved member
    // signatures can be reused when the resolver itself is cached.
    private var resolvedMemberFunctions: MutableMap<String, FunctionAndTypes?>? = null

    fun findMemberFunctionWithTypeByTransformedName(memberName: String): FunctionAndTypes? {
        val resolved = resolvedMemberFunctions ?: mutableMapOf<String, FunctionAndTypes?>().also { resolvedMemberFunctions = it }
        return resolved.getOrPut(memberName) {
            clazz.findMemberFunctionWithEnclosingTypeNameByTransformedName(memberName)
                .resolveTypes()
        }
    }
