...
open class ObjectType(val clazz: ClassDefinition, val arguments: List<DataType>, override val isNullable: Boolean = false, val superTypes: List<ObjectType>) : DataType {
    ...
    // Owned by Interpreter.memberResolverFor. Only set for concrete type arguments,
...
fun TypeNode.toPrimitiveDataType(symbolTable: SymbolTable) = when(this.name) {
    ...
    // BlueK approximates Float with Double; `1.5f` literals are Double values as well.
    "Float" -> if (isNullable) symbolTable.NullableDoubleType else symbolTable.DoubleType
